...
to parse the initial state and program:

```go
func parseRegisters(input string) (regA, regB, regC int, program []int) {
    for _, line := range strings.Split(input, "\n") {
        if strings.HasPrefix(line, "Register A:") {
            regA = utils.AtoiNoErr(strings.TrimSpace(line[12:]))
        } else if strings.HasPrefix(line, "Register B:") {
            regB = utils.AtoiNoErr(strings.TrimSpace(line[12:]))
        } else if strings.HasPrefix(line, "Register C:") {
            regC = utils.AtoiNoErr(strings.TrimSpace(line[12:]))
